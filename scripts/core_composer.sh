...
if command -v composer >/dev/null 2>&1; then
    ...
    COMPOSER_ALLOW_SUPERUSER=1 composer --version | tee -a "${LOG_FILE}" || log_fail "Composer was installed but could not be executed from PATH."
...
expected_signature="$(curl -fsSL https://composer.github.io/installer.sig)" || log_fail "Could not download Composer installer signature. Composer was not installed."
php -r "copy('https://getcomposer.org/installer', '/tmp/composer-setup.php');" || log_fail "Could not download Composer installer. Composer was not installed."
actual_signature="$(php -r "echo hash_file('sha384', '/tmp/composer-setup.php');")" || log_fail "Could not calculate Composer installer signature. Composer was not installed."
